...
| `REDIRECT_ALLOWED_HOSTS` | _(empty)_ | Comma-separated hosts a user may be redirected to after signing in, for ForwardAuth apps outside `COOKIE_DOMAIN`. Each entry covers that domain and its subdomains, so `example.com` allows `app.example.com`. The `BASE_URL` host, `ADMIN_URL` host, and `COOKIE_DOMAIN` are always allowed. Usually set in the admin UI instead, under Settings - Access control - Protected app domains; entries from both sources are combined. |
